...
<div class="foodlist_center">
    ...
    <table id="f_table" class="table table-hover" align="center" width=900>
		...
		<tbody>
				...
				<c:forEach var="fb" items="${staylist }">
					<tr>
						<td>${no1}</td>
							<td><a href="${path }/staycontent.do/stay_no/${fb.stay_no}/pageNum/${pp.currentPage}">
								${fb.stay_title}
								</a></td>
							<td>${fb.username}</td>
							<td><fmt:formatDate value="${fb.stay_regdate}" 
			...
			<center>
				<c:if test="${not empty keyword}">
					<c:if test="${pp.startPage > pp.pagePerBlk }">
						<li><a href="${path }/staylist.do/pageNum/${pp.startPage - 1}?search=${search}&keyword=${keyword}">이전</a></li>
					</c:if>
				...
				<c:if test="${empty keyword}">
					<c:if test="${pp.startPage > pp.pagePerBlk }">
						<li><a href="${path }/staylist.do/pageNum/${pp.startPage - 1}">이전</a></li>
					</c:if>
					<c:forEach var="i" begin="${pp.startPage}" end="${pp.endPage}">
						<li <c:if test="${pp.currentPage==i}">class="active"</c:if>><a
							href="${path }/staylist.do/pageNum/${i}">${i}</a></li>
					</c:forEach>
					<c:if test="${pp.endPage < pp.totalPage}">
						<li><a href="${path }/staylist.do/pageNum/${pp.endPage + 1}">다음</a></li>
					</c:if>
			  </c:if>
			</center>
		    </div>	
		...
		<form align="center" action="${path}/staylist.do/pageNum/1">
			<select name="search">
				<option value="stay_title"
					<c:if test="${search=='stay_title'}">selected="selected" </c:if>>제목</option>
				<option value="stay_content"
					<c:if test="${search=='stay_content'}">selected="selected" </c:if>>내용</option>
