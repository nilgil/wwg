		...
		<div class="table">
					...
					<c:forEach var="qna" items="${qnalist}">
						...
						<tbody>
							<tr>
								...
								<td><c:if test="${qna.qna_re_lev != 0}">
										<c:forEach var="q" begin="1" end="${qna.qna_re_lev}">
							&nbsp;&nbsp;
						</c:forEach>
										<img src="/img/qna/reply.png" width="22" height="22">
									</c:if> <a
									href="${path}/qna_detail/qna_no/${qna.qna_no}/page/${page}/no/${no1}">${qna.qna_title}</a></td>
			...
			<div class="qnalist_write_btn">
				...
				<s:authorize access="hasAnyRole('ROLE_ADMIN','ROLE_USER')">
					<a href="qnawriteform.do"><input class="btn btn-dark"
         ...
         &nbsp;${a}&nbsp;
				...
				<c:if test="${page < maxpage}">
					<a href="qnalist.do?page=${page+1}">[다음]</a>
				</c:if>
			</div>
				...
				<form action="qnalist.do">
					<input type="hidden" name="page" value="${page}"> <select
						...
						<option value="qna_title"
							<c:if test="${search=='qna_title'}"> selected="selected"</c:if>>
							제목</option>
						<option value="qna_content"
							<c:if test="${search=='qna_content'}"> selected="selected"</c:if>>
							내용</option>
						<option value="username"
							<c:if test="${search=='username'}"> selected="selected" </c:if>>
							작성자</option>
						<option value="qna_titcon"
							<c:if test="${search=='qna_titcon'}"> selected="selected" </c:if>>
							제목+내용</option>
					</select> <input type="text" name="keyword"> <input
						class="btn btn-dark" type="submit" value="검색">
				</form>
			</div>
